...
const formatCommand = (suffix) => chalk.cyanBright.bold(`${netlifyCommand()} ${suffix}`);
...
const readLocalMigrations = async (migrationsDirectory) => {
    let entries;
    try {
...
const toEntry = (m) => ({ version: m.version, name: m.name });
export const computeStatus = (applied, local) => {
...
const connectionStringHasCredentials = (connectionString) => {
    try {
        const url = new URL(connectionString);
...
const isNetlifyDatabasePackageInstalled = async (projectRoot) => {
    try {
...
const fetchBranchConnectionString = async (ctx, branchId) => {
    const token = ctx.accessToken.replace('Bearer ', '');
    const url = new URL(`${ctx.basePath}/sites/${encodeURIComponent(ctx.siteId)}/database/branch/${encodeURIComponent(branchId)}`);
...
const fetchSiteDatabase = async (ctx) => {
    const token = ctx.accessToken.replace('Bearer ', '');
...
const primary = (emoji, text) => {
    log(`  ${emoji} ${text}`);
};
const secondary = (text) => {
    log(chalk.gray(`${INDENT}${text}`));
};
const renderPretty = (params) => {
    ...
    if (connectionString) {
        ...
        if (!showCredentials && connectionStringHasCredentials(connectionString)) {
            secondary(`To reveal the full connection string (including credentials), run ${formatCommand('db status --show-credentials')}`);
        }
        else {
        ...
        secondary(`It starts automatically when you run ${formatCommand('dev')}. Run that in a new terminal and try this command again`);
        logConnectCommands();
    }
    log('');
    ...
    log('');
    log(chalk.gray('Note that these migrations cannot be removed or edited. To change anything, you should generate a new migration.'));
    log('');
    ...
    if (status.missingOnDisk.length > 0 || status.outOfOrder.length > 0) {
        ...
        if (status.outOfOrder.length > 0) {
            log(`  Out of order: ${status.outOfOrder
                .map((m) => chalk.red(`${m.name} (version ${String(m.version)} <= max applied ${String(m.maxApplied)})`))
                .join(', ')}`);
            log('');
            log(chalk.gray(`Run ${formatCommand('db migrations reset')} to delete these local-only migrations, then generate them again with a higher prefix.`));
        }
    }
};
export const statusDb = async (options, command) => {
