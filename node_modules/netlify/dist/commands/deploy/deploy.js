...
const prepareProductionDeploy = async ({ api, siteData, options, command }) => {
    if (isObject(siteData.published_deploy) && siteData.published_deploy.locked) {
        ...
        if (!isInteractive()) {
            return logAndThrowError(`Deployments are "locked" for production context of this project.\n\n` +
...
const createSiteWithFlags = async (options, command, site) => {
    ...
    if (!options.team) {
        const team = resolveTeamForNonInteractive(accounts, `netlify deploy --site-name${siteName ? ` ${siteName}` : ' <SITE_NAME>'} --team <TEAM_SLUG>`);
    ...
    try {
        ...
        if (error_.status === 422 && siteName) {
            ...
            try {
                ...
                return logAndThrowError(`Failed to create site "${suffixedName}": ${retryError.status}: ${retryError.message}`);
            }
        }
...
const anonymousDeploy = async (options, command) => {
    const { workingDir } = command;
    const { site, config } = command.netlify;
    const dirHasFiles = async (dir) => {
        if (!dir)
            return false;
        try {
    ...
    const checkForFunctions = async () => {
        ...
        const hasFunctions = (await dirHasFiles(functionsFolder)) ||
        ...
        if (hasFunctions || hasEdgeFunctions) {
            log(`\n${NETLIFYDEVERR} This project includes ${hasFunctions ? 'serverless functions' : ''}${hasFunctions && hasEdgeFunctions ? ' and ' : ''}${hasEdgeFunctions ? 'edge functions' : ''} which require authentication.`);
    ...
    try {
        ...
        const dropError = error;
        if (dropError.status === 429) {
            const loginCommand = isInteractive()
                ? chalk.cyanBright('netlify login')
                : chalk.cyanBright('netlify login --request <message>');
            return logAndThrowError(`You've reached the daily limit for anonymous deploys. Run ${loginCommand} to sign up or log in, then retry your deploy.`);
        }
        throw error;
    }
    ...
    const deploy = await waitForDropDeploy(dropApiOptions, deployInfo.id, deployInfo.deploy_id, options.timeout ? options.timeout * 1000 : DEFAULT_DEPLOY_TIMEOUT);
...
export const deploy = async (options, command) => {
    ...
    if (options.allowAnonymous) {
        ...
        if (token) {
            ...
            if (!hasSiteData && !options.createSite) {
                return logAndThrowError(`No project linked. Use ${chalk.cyanBright('--create-site <name>')} to create a new site, or ${chalk.cyanBright('--site <name-or-id>')} to deploy to an existing project.`);
            }
        }
        else {
    ...
    if (!authToken && !isInteractive()) {
        return logAndThrowError(`Authentication required. NETLIFY_AUTH_TOKEN is not set and ${chalk.cyanBright('`netlify login --request <message>`')} can be used to authenticate.\nAlternatively, use ${chalk.cyanBright('--allow-anonymous')} to deploy without an account.`);
    }
