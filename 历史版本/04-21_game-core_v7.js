...
var gameState = {
    ...
    settings:{sound:true,autoSave:true,animations:true,bgmVolume:0.5},
    _equipUid:1,
};
...
GameCore.prototype.cultivate = function() {
    ...
    gameState.energy = Math.max(0, gameState.energy - 1); gameState.stats.totalCultivations++;
...
GameCore.prototype.gainGold = function(amt) { gameState.gold += amt; gameState.stats.totalGold += amt; };
...
GameCore.prototype.getTotalStat = function(base) { return gameState[base] + (gameState.equipBonus[base] || 0); };
...
GameCore.prototype.calcDamage = function(atk, def, luck, isPlayer) {
    ...
    if (crit) { dmg = Math.floor(dmg * 1.5); gameState.stats.totalCrits++; }
...
GameCore.prototype.startBossBattle = function() {
    ...
    this.emit('battleStart', {enemy: gameState.currentEnemy, isBoss: true});
};
...
GameCore.prototype.startBattle = function() {
    ...
    this.emit('battleStart', {enemy: gameState.currentEnemy, isBoss: tpl.isBoss});
};
...
GameCore.prototype.autoBattleTick = function() {
    ...
    gameState.currentEnemy.hp -= pR.dmg;
    this.emit('playerAttack', {dmg: pR.dmg, crit: pR.crit});
    ...
    gameState.hp -= eR.dmg;
    this.emit('enemyAttack', {dmg: eR.dmg, crit: eR.crit});
...
GameCore.prototype.enemyTurn = function() {
    ...
    var eR = this.calcDamage(e.atk, 0, 0); gameState.hp -= eR.dmg;
    this.emit('enemyAttack', {dmg: eR.dmg, crit: eR.crit});
    if (gameState.hp <= 0) { this.loseBattle(); return; }
    gameState.battleTurn = 'player'; gameState.playerActionLock = false;
};
...
GameCore.prototype.winBattle = function() {
    ...
    var healAmt = Math.floor(gameState.maxHp * 0.1);
    ...
    // Boss必掉1宝石
    if (e.isBoss) { gameState.gem += 1; }
    // 装备掉落
    var drop = this.rollEquipDrop();
    if (gameState.buffRage > 0) gameState.buffRage--;
    var result = {exp: e.exp, gold: e.gold, healAmt: healAmt, luckDrop: luckDrop, equipDrop: drop, isBoss: e.isBoss};
...
GameCore.prototype.useItem = function(itemId) {
    ...
    if (it.effect === 'heal_15pct') { var h = Math.min(Math.floor(maxHp * 0.15), maxHp - gameState.hp); gameState.hp = Math.min(maxHp, gameState.hp + Math.floor(maxHp * 0.15)); result.healAmt = h; }
    if (it.effect === 'heal_40pct') { var h2 = Math.min(Math.floor(maxHp * 0.4), maxHp - gameState.hp); gameState.hp = Math.min(maxHp, gameState.hp + Math.floor(maxHp * 0.4)); result.healAmt = h2; }
    if (it.effect === 'energy_50') { gameState.energy = Math.min(gameState.maxEnergy, gameState.energy + 50); result.energyAmt = 50; }
    if (it.effect === 'exp_200') { this.gainExp(200); result.expAmt = 200; }
